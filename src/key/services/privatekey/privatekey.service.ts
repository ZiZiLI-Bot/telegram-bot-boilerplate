...
export class PrivatekeyService {
  ...
  async reverseKey(pk: string): Promise<string> {
    ...
    const decryptedPk = Buffer.concat([
      decipher.update(encryptedPk),
      decipher.final(),
    ]);
  ...
  async resetUserKey(userId: number): Promise<ResetUserKeyResult> {
    ...
    await this.keyRepo
      ...
      .update(Key)
      .set({
        walletPk: this.encryptData(
          base58.encode(newKey.secretKey).toString(),
          randomBytes(16),
        ),
        walletAddress: newKey.publicKey.toBase58(),
      })
      .where('id = :id', { id: k.id })
      .execute();
